...
import { makeNotifyConventionSummary } from "../../domains/agency/use-cases/notifications/NotifyConventionSummary";
...
const notifyConventionSummaryScript = async () => {
  ...
  const notifications = await makeNotifyConventionSummary({
    uowPerformer,
    deps: {
      timeGateway,
...
export const triggerNotifyConventionSummaryScript = ({
  exitOnFinish,
}: {
  ...
  handleCRONScript({
    ...
    config,
    script: notifyConventionSummaryScript,
